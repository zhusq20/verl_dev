...
class RayPPOTrainer_v2(object):
    ...
    def fit(self):
        ...
        self.global_steps += 1
        prev_batch = [dict()] * self.config.trainer.total_epochs

        for epoch in range(self.config.trainer.total_epochs):
            # for batch_dict in self.train_dataloader:
            for batch_idx, batch_dict in enumerate(self.train_dataloader):
            # 处理 batch_dict
                print(f"Batch {batch_idx}: {batch_dict}")
                ...
                with _timer('step', timing_raw):
                    ...
                    prev_batch[epoch][batch_idx] = batch

                    if epoch > 0:
                        chunked_batch = prev_batch[epoch-1][batch_idx].chunk(2)
                        chunked_batch_now = prev_batch[epoch][batch_idx].chunk(2)
                        batch = chunked_batch_now[0].concat([chunked_batch_now[0], chunked_batch[1]])
                    else:
                        chunked_batch_now = prev_batch[epoch][batch_idx].chunk(2)
                        batch = chunked_batch_now[0]
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # if self.replay_buffer.batch is not None and self.replay_buffer.batch.batch_size[0] > 0:
                batch, self.prompt_buffer = batch.concat_partial(self.config.data.max_prompt_length ,\
                                                         self.tokenizer.pad_token_id, \
                                                         [batch, self.prompt_buffer, self.replay_buffer], self.actor_rollout_wg.world_size)
                # print("batch meta_info", batch.meta_info)

                # pop those keys for generation
                # 在这里给partial_rollout的问题做长度裁剪，使其与输入的长度一致
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids', 'reason'])
                ...
                with _timer('step', timing_raw):
                    ...
                    # 处理partial rollout
                    tmp = batch.partial_rollout(self.actor_rollout_wg.world_size)
                    self.replay_buffer = tmp[0] # tmp[0]有可能是None dataproto
                    if tmp[1].batch.batch_size[0] > 0:
